function addInputField(t) {
    if (count == limits) alert("You have reached the limit of adding " + count + " inputs");
        ...
        e.innerHTML = "<td><input type='text' name='product_name' onkeypress='invoice_productList(" + count + ");' class='form-control productSelection' placeholder='Product Name' id='" + a + "' required tabindex='"+tab1+"'><input type='hidden' class='autocomplete_hidden_value product_id_" + count + "' name='product_id[]' id='product_id'/></td> <td><input type='text' name='available_quantity[]' id='' class='form-control text-right available_quantity_" + count + "' value='' tabindex='"+tab2+"' readonly='readonly' /></td><td> <input type='text' placeholder='0.00' name='product_quantity[]' class='total_qty_" + count + " form-control text-right' autocomplete='off' id='total_qty_" + count + "' onkeyup='quantity_calculate(" + count + ");' onchange='quantity_calculate(" + count + ");' tabindex='"+tab3+"' required /></td><td><input type='text' name='product_rate[]' onkeyup='quantity_calculate(" + count + ");' onchange='quantity_calculate(" + count + ");' value='' id='item_price_" + count + "' class='item_price_"+count+" form-control text-right' required /></td><td><select name='discount[]' id='discount_" + count + "' class='form-control' tabindex='"+tab4+"' ><option value='Slitting'>Slitting</option><option value='Printing'>Printing</option><option value='Cutting'>Cutting</option><option value='Store'>Store</option><option value='Other'>Other</option></select></td><td class='text-right'><input class='total_price form-control text-right' type='text' name='total_price[]' id='total_price_" + count + "' value='' readonly='readonly'/></td><input type='hidden' id='total_discount_" + count + "' class='total_tax_" + count + "' /><input type='hidden' id='all_discount_" + count + "' class='total_discount'/><td><button style='text-align: right;' class='btn btn-danger' type='button' tabindex='"+tab5+"' value='Delete' onclick='deleteRow(this)'>Delete</button></td>",
