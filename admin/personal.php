...
<head>
		...
		<!-- Title -->
		<title>PERSONAL MONEY LIST</title>
				...
				<div class="content-area py-1">
					<div class="container-fluid">
						...
						<div class="box box-block bg-white">
							<h5 class="mb-1">List of cash collected by expectiate</h5>
							...
							<table class="table table-bordered dataTable" id="s_i_l">
							  ...
							  {
								$sql = "SELECT * FROM `petty_cash_tb` WHERE (`details` LIKE '%personal%' or  `other_cat` LIKE 'personal' ) AND `date` BETWEEN '$_POST[start]' AND '$_POST[end]' ";
							  }else{
							   $newEndingDate = date("Y-m-d", strtotime(date("Y-m-d", strtotime(date("Y-m-d"))) . " - 1 year"));
								$sql = "SELECT * FROM `petty_cash_tb` WHERE (`details` LIKE '%personal%'  or `other_cat` LIKE 'personal') ";
							  }
