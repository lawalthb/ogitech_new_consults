...
$response = array();
    if (isset($_POST['edit'])) {
        $documnents_id=strip_tags(trim($_POST['edit']));
...
if((!empty($_POST['customer_name'])) || (!empty($_POST['new_customer'])) && (!empty($_POST['product_id'][0])) ) {
	...
	$invoice_sqlq=$conn->query($invoice_sql);
	 ...
	 echo  $items_lines=sizeof($_POST['product_id']);
	die();
	...
	echo $inv_line_sql = "INSERT INTO invoice_line_items (invoice_no_id,product_id,product_quantity,product_rate,purchase_rate,total_price, commet)
