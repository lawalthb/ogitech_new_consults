...
//	$document_type= $ac_type.$inv_type ;
	$invoice_sql = "UPDATE `documents` SET `document_date` = '$datepicker_invoice_exp', `document_comments` ='$details',`inserted_at` = CURRENT_TIMESTAMP, `auth_id` = '$auth_id' WHERE `documents`.`documnents_id` = '$doc_id'  ";  
			...
			//customer
	 $inv_pay_sql = "UPDATE `ledger_entries` SET `account_id` = '$paid2ru', `cr_amount` = '$amount_in', `against` = '$particular',  `sub_acct_group_id` = '$act_type_id', `act_grp_id` = '$act_grp_id' WHERE `ledger_entries`.`id` = '$dr_id'";  
    	 
	

//bank
	 $inv_pay_sql2 = "UPDATE `ledger_entries` SET `account_id` = '$particular', `dr_amount` = '$amount_in', `against` = '$paid2ru',  `sub_acct_group_id` = '$act_type_id2', `act_grp_id` = '$act_grp_id2' WHERE `ledger_entries`.`id` = '$cr_id'";  
