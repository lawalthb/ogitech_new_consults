...
<head>
		...
		<!-- Title -->
		<title>Edit receipt Voucher</title>
                            ...
                            <table class="table table-bordered table-hover" id="normalinvoice">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center">Amount Paid <i class="text-danger">*</i>(In) </th>                                        
                                        <th class="text-center">Select Customer<i class="text-danger">* </i>(Crediting) </th>
								...
								$resultc = $conn->query($c); ?>
                                        ...
                                        <td >
										<input type="text" required autocomplete="off" name="particular" class="customerSelection form-control" value="<?=$rowd['cus_name']?>" placeholder='paid by?' id="customer_name" required  />

<input id="customer_id" class="hidden_value" value="<?=$rowd['cus_id']?>" type="hidden" name="particular">
...
<?php while($rowc = $resultc->fetch_assoc()) { ?>
	<option value="<?php echo $rowc['cus_id'];?>"><?php echo $rowc['cus_name'];?> </option>
<?php } ?>	
</select> --> <span><a href="add-customer.php">New</a></span>
									...
									<tr >
                                        <td colspan="4"><textarea   style="width:100%" name="doc_comment" placeholder="Enter Note here"><?=$rowd['document_comments']?>, <?=$rowd['cr_amount']?></textarea>
			   ...
			   function submitForm()
				{
					...
					$.ajax({
						...
						type : 'POST',
						url  : 'modal/edit_receiptModel.php',
						data : data,
						dataType : 'json',
						...
						//alert(data.status);
							...
							else if(data.status==='successfully')
							{
                                    ...
                                    setTimeout('$("#addexpvoucher").fadeOut(500, function(){ window.location = "sales_pays.php?ac_type=7&showall=1" }); ',800);
