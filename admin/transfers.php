...
//die();
  if ($conn->query($del) === TRUE) {
    ...
    header("location: transfers.php?d=del&ac_type=4&showall=1");   
   }else{
    header("location: transfers.php?d=n&ac_type=4&showall=1");   
  }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    if($_GET['ac_type']==4 ){
                            echo 'Transfer Money';
                    }elseif($_GET['ac_type']==6 ){

                          echo 'Credit Note ';
                    }else{echo  "Debit Note";}
                    ?></title>
...
die('<script>alert("Invalid link")</script>');
            ...
            <ol class="breadcrumb no-bg mb-1">
              ...
              <li class="breadcrumb-item active"><?php  if($_GET['ac_type']==4  ){
                  if(!isset($_GET['showall'])){  ?>
                <a href="transfer.php?ac_type=4&showall=1" >Transferred List</a>
              <?php }else{?>
                <a href="transfer.php?ac_type=4" >Transferred Note</a><?php
                } }elseif($_GET['ac_type']==6){
                  if(!isset($_GET['showall'])){  ?>
                    <a href="transfer.php?ac_type=6&showall=1" >Debit or Credit Note</a>
                  <?php }else{?>
                    <a href="transfer.php?ac_type=6" >Debit or Credit Note</a><?php
                    }}elseif($_GET['ac_type']==6 ){  ?>
                        <a href="transfer.php?ac_type=6&showall=1" >Debit or Credit Note</a>
                      <?php }?>
            ...
            <div class="box box-block bg-white">
              <h5 class="mb-1">Exporting Transfers <?php  if(!empty($_SESSION['user_type_id']) &&( $_SESSION['user_type_id']!='2' and $_SESSION['user_type_id']!='4'  ))  { ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp; &nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;
              <?php }
                if($_GET['ac_type']==4  and $_SESSION['user_type_id']!='3' ){ ?>
                  <input  type="button"  class="btn btn-success checkbox_account" onClick="JAVASCRIPT:window.location ='add-transfer.php?ac_type=4&inv_type=1'")  name="customer_confirm"  value="New Transfer" >
<?php
                    }elseif($_GET['ac_type']==6 and $_SESSION['user_type_id']!='3' ){
?>
                        <input  type="button"  class="btn btn-success checkbox_account" onClick="JAVASCRIPT:window.location ='add-dr-cr.php?ac_type=6&inv_type=1'")  name="customer_confirm"  value="Add Debit or Credit Note" >
                   <?php }
?>
                        </h5>
              ...
              <table class="table table-striped table-bordered" id="table-2">
                <thead>
                  <tr>
                    ...
                    if($_GET['ac_type']!=5 ){  ?>
                    <th>note No.</th>
                    <?php }else{ echo  "<th>Voucher No.</th>";} ?>
                    <th><?php echo $lang['Date'];?></th>

                                       <?php
                    if($_GET['ac_type']!=5 ){  ?>
                    <th>To</th>
                    <?php }else{ echo  "Particular";} ?></th>
                ...
                <tbody>
                 ...
                 $sql = "SELECT c.*, inv.* FROM ledger_entries as inv, documents as c where c.documnents_id=inv.documents_id and inv.sub_acct_group_id =2  ORDER BY `inv`.`id` DESC";
                                  ...
                                  $sql = "SELECT  c.*, inv.* FROM ledger_entries as inv, documents as c where c.documnents_id=inv.documents_id  and c.document_type=41  ORDER BY `inv`.`id` DESC";
                                }

                                if(isset($_POST['start']) and isset($_POST['end'])){
                                  $sql = "SELECT c.*, inv.* FROM ledger_entries as inv, documents as c where c.documnents_id=inv.documents_id  and c.document_date BETWEEN '$_POST[start]' AND '$_POST[end]' ORDER BY `inv`.`id` DESC";
                        }}elseif($_GET['ac_type']==6 ){
                                                if(isset($_GET['showall'])){
                                  
                  $sql = "SELECT c.*, inv.* FROM ledger_entries as inv, documents as c where c.documnents_id=inv.documents_id   ORDER BY `inv`.`id` DESC";
                            }elseif(!isset($_GET['showall'])){
                         $sql = "SELECT c.*, inv.* FROM ledger_entries as inv, documents as c where c.documnents_id=inv.documents_id  and c.document_type=91  ORDER BY `inv`.`id` DESC";
                                }
          ...
          $sql = "SELECT c.*, inv.* FROM ledger_entries as inv, documents as c where c.documnents_id=inv.documents_id and inv.sub_acct_group_id = 2  and c.document_date BETWEEN '$_POST[start]' AND '$_POST[end]' ORDER BY `inv`.`id` DESC";
                                                  ...
                                                  }}elseif($_GET['ac_type']==5  ){ 

                                                    if(isset($_GET['showall'])){
                                                      
                                     $sql = "SELECT c.*, inv.* FROM ledger_entries as inv, documents as c where c.documnents_id=inv.documents_id and inv.sub_acct_group_id =12 ORDER BY `inv`.`id` DESC";
                                                    }elseif(!isset($_GET['showall'])){
                                                      $sql = "SELECT c.*, inv.* FROM ledger_entries as inv, documents as c where c.documnents_id=inv.documents_id and inv.sub_acct_group_id =5  ORDER BY `inv`.`id` DESC";
                                                    }
                    
                                                    if(isset($_POST['start']) and isset($_POST['end'])){
                                                      $sql = "SELECT c.*, inv.* FROM ledger_entries as inv, documents as c where c.documnents_id=inv.documents_id and inv.sub_acct_group_id = 5  and c.document_date BETWEEN '$_POST[start]' AND '$_POST[end]' ORDER BY `inv`.`id` DESC";
                                            }}
                ...
                if ($result->num_rows > 0) 
                {
                  ...
                  <tr>
					...
					<td><?php $against=  $row['against'];
					$sql7 = "SELECT * FROM `customers` WHERE `cus_id` = $against ";
                    ...
                    <td><a class="btn btn-success btn-sm" href="invoice.php?i=<?php echo $row['documnents_id'];?>" title="View"><i class="ti-eye mr-0-5"></i><?php echo $lang['View'];?></a> 
                                       <?php  if(!empty($_SESSION['user_type_id']) &&( $_SESSION['user_type_id']!='2' and $_SESSION['user_type_id']!='4'  and $_SESSION['user_type_id']!='3'))  { ?>
										...
										<a class="btn btn-secondary btn-sm" href="edit_purchase-payment.php?sales_income_edit=<?php echo $row['documents_id'];?>" title="Edit"><i class="ti-pencil mr-0-5"></i><?php echo $lang['Edit'];?></a> 
                ...
                <tfoot>
                  <tr>
                    ...
                    if($_GET['ac_type']!=5 ){  ?>
                    <th>To</th>
                    <?php }else{ echo  " <th>Particular</th>";} ?>
...
function Delete(x,y,p) {
 //alert(x);
  ...
  if (confirm("Are you sure you want to delete ")) {
   window.location.href = "purchase_pays.php?i="+x+"&ac_type="+y+"&p="+p;
  } else {
