...
if(isset($_GET['e']) && !empty($_GET['e'])){
	...
	$del = "DELETE FROM `petty_cash_tb` WHERE `petty_cash_tb`.`petty_id` = '$e' "; 
	$del = "DELETE FROM `sales_payments_tb` WHERE `sales_payments_tb`.`invoice_payment_id` = '$e' ";
	$del = "DELETE FROM `invoice_payment_detail` WHERE `invoice_payment_detail`.`invoice_payment_id` = '$e' ";
	if ($conn->query($del) === TRUE) {
		header("location: petty-vouchers.php?d=del");		
	} else {
		header("location: petty-vouchers.php?d=n");		
	}
}
?>
<!DOCTYPE html>
...
<head>
					...
					<div class="container-fluid">
						...
						<div class="table-responsive bg-white" style="margin-top: 10px">
							...
							<table class="table table-striped table-bordered dataTable" id="table-2">
								...
								<tbody>
                                <?php 
								$sql = "SELECT * FROM `petty_cash_tb`  where  `type` LIKE 'out' ORDER BY `voucher_no` DESC ";
								...
								if ($result->num_rows > 0) 
								{
									...
									echo $row['particular'];
                                        ...
                                        <td><?php 
										 $sql2 = "SELECT  * from `auth` where auth_id=$row[user_id] ";
	...
	if ($result2->num_rows > 0) 
	{
		...
		{ 
			$auth_id =$row2['auth_id'];
			

			 $full_name =$row2['full_name'];	
			echo $auth_username =$row2['auth_username'];			
		}
	}
										?></td>
                                        ...
                                        <a class="btn btn-secondary btn-sm" href="edit-petty-vouchers.php?e=<?php echo $row['cus_last_id'];?>" title="Edit"><i class="ti-pencil mr-0-5"></i><?php echo $lang['Edit'];?></a> 
                                        <a class="btn btn-danger btn-sm" href="JAVASCRIPT:onclick=Delete(<?php echo $row['cus_last_id'];?>)"  title="Delete"><i class="ti-trash mr-0-5"></i><?php echo $lang['Delete'];?></a>
                                        <?php } ?>
                                        </td>
...
function Delete(x) {
  ...
  if (confirm("Are you sure you want to delete !")) {
   window.location.href = "petty-vouchers.php?e="+x;
  } else {
