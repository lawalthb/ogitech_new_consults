...
die('<script>alert("Invalid link")</script>');
                  ...
                  //get document details of seletec invoice
                    if($_GET['ac_type']==1 and $_GET['inv_type']==1 and isset($_GET['i']) ){
                        $doc_id  = $_GET['i'];
                        $cus_id  = $_GET['cus_id'];
   $q_doc= "SELECT `documents`.*, `ledger_entries`.*, `customers`.*
 FROM `documents`
     , `ledger_entries`
     , `customers`
 WHERE(  `ledger_entries`.`documents_id` = $doc_id and `documents`.`documnents_id` = $doc_id and `customers`.`cus_id`=$cus_id);";
$r_doc = $conn->query($q_doc);
$row_doc = $r_doc->fetch_assoc();
 $edit_doc_id = $row_doc['document_no'];
}
                        ...
                        <div id="error"></div>
                             ...
                             <div class="col-sm-8" >
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-6">
                                       <input type="text" size="100" autocomplete="off"  onkeyup="chk_inv(this.value)"   name="paper_inv_no" value="<?php if(isset($row_doc['document_no'])){echo $row_doc['document_no'];}; ?>" class="form-control" placeholder='Enter Invoice Number' id="paper_inv_no" required  />
              ...
              <?php  }
                    ?>
                                    ...
                                    <div class="col-sm-6">
                                    <input type="text" size="100" autocomplete="off"   name="customer_name" class="customerSelection form-control" placeholder='' id="customer_name" required value="<?=$row_doc['cus_name']?>"  /><a href="add-account3.php">[New]</a>

                                        <input id="customer_id" class="hidden_value" type="hidden" value="<?=$row_doc['account_id']?>" name="customer_id">
              ...
              <?php  }
                    ?>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-4">
                                <input type="text" class="form-control" autocomplete="off" required  id="datepicker_invoice_date" value="<?=$row_doc['document_date']?>" name="datepicker_invoice_date" placeholder="yyyy-mm-dd">
                                <span class="input-group-addon"><i class="fa fa-calendar-o"></i></span>
                            </div>
                        </div>        <input id="auth_id"  type="hidden" name="auth_id" value="<?php echo $_SESSION['auth_id'];?>">
                                     <input id="edit"  type="text" name="edit" value="<?=$doc_id?>">

                        <div class="table-responsive" style="margin-top: 10px">
                        <?php
                         $q_prod= "SELECT p.*, pli.* from invoice_line_items as pli, product as p 
                         where p.product_id=pli.product_id and pli.invoice_no_id= $doc_id";
                    $r_prod = $conn->query($q_prod);
                    $row_prod = $r_prod->fetch_assoc();
                     $edit_prod_id = $row_prod['document_no'];
                        ?>
                            <table class="table table-bordered table-hover" id="normalinvoice">
                                ...
                                <tbody id="addinvoiceItem">
                                    <tr>
                                        <td style="width: 220px">
                                            <input name="product_name[]" value="<?=$row_prod['product_name']?>" onkeypress="invoice_productList(1);" class="form-control productSelection" placeholder="<?php echo $lang['Products'];?>" required  id="product_name" autocomplete="off" tabindex="1" type="text">
                                           <input type="text" class="autocomplete_hidden_value product_id_1" value="<?=$row_prod['product_id']?>" name="product_id[]" id="product_id"/>
                                        </td>
                                        ...
                                        <td>
                                            <input name="product_quantity[]" value="<?=$row_prod['product_quantity']?>" autocomplete="off" class="total_qty_1 form-control" id="total_qty_1" onkeyup="quantity_calculate(1);" required onchange="quantity_calculate(1);" placeholder="0.00" tabindex="3" type="text">
                                        </td>
                                        <td style="width: 85px">
                                            <input name="product_rate[]" value="<?=$row_prod['product_rate']?>" onkeyup="quantity_calculate(1)" onchange="quantity_calculate(1);" value="" id="item_price_1" class="item_price_1 price_item form-control" tabindex="7"   type="text">
                                        ...
                                        <td>
                                            <input name="discount[]" id="discount_1" class="form-control" value="" autocomplete="off" min="0" tabindex="4"   onkeyup="quantity_calculate(1);"  onchange="quantity_calculate(1);" placeholder="0.00" type="hidden">
                                             ...
                                             <input name="commet[]" value="<?=$row_prod['commet']?>" id="discount4" class="form-control" value="" autocomplete="off" min="0" tabindex="4"  placeholder="if any comment" type="text">
                                ...
                                <tfoot>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-right">
                                            <input id="grandTotal"  class="form-control" name="grand_total_price" value="<?=$row_prod['total_price']?>" tabindex="-1" readonly type="text">
                                        </td>
                                    </tr>
                                    <tr>
               ...
               function submitForm()
                {
                    ...
                    $.ajax({
                        ...
                        type : 'POST',
                        url  : 'modal/add_invoiceModel2.php',
                        data : data,
                        dataType : 'json',
